...
export async function action({ request }: { request: Request }) {
  ...
  if (!response.ok) {
    return { error: "Error al crear el atributo: " + response.status };
  }
  return { resultado: "Atributo creado correctamente" };
}

// 🔹 Componente Crear Atributo
export default function CrearAtributo() {
  ...
  // 🔹 Si se creó correctamente, volvemos atrás
  React.useEffect(() => {
    if (actionData?.resultado) navigate(-1);
  }, [actionData, navigate]);

  return (
    <Dialog title="Crear atributo" onClose={() => navigate(-1)}>
      <Form method="post">
  <input type="hidden" name="idAtributo" value={0} />
  <div className="k-form-field">
        ...
        <div className="k-form-field">
          ...
          <input className="k-textbox" name="valorMinimo" required />
        </div>
        <div className="k-form-field">
          <label>Valor máximo:</label>
          <input className="k-textbox" name="valorMaximo" required />
        </div>
        <div className="k-form-field">
          ...
          {valoresPosibles.map((valor, idx) => (
            <div key={idx} style={{ display: "flex", gap: 4, alignItems: "center" }}>
              ...
              {valoresPosibles.length > 1 && (
                <Button
                  type="button"
                  onClick={() => setValoresPosibles(valoresPosibles.filter((_, i) => i !== idx))}
                  themeColor="base"
                >
                  -
                </Button>
